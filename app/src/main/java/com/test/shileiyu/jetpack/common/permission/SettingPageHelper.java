...
/**
 * 以下内容参考 开源库 AndPermission
 * <p>
 * 提供了主流手机厂商跳转到开启权限页面的细分处理
 *
 * @author lanche.ysl
 * @date 2018/10/16 下午4:49
 */
public class SettingPageHelper {
    private SettingPageHelper() {
    }

    public static final int CODE_SETTING_REQUEST = 0x878;
    ...
    public static void toPermissionPage(Activity activity) {
        ...
        try {
            activity.startActivityForResult(intent, CODE_SETTING_REQUEST);
        } catch (Exception e) {
            intent = defaultApi(activity);
            activity.startActivityForResult(intent, CODE_SETTING_REQUEST);
        }
